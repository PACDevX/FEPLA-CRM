...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($nombre) && !empty($apellido1) && !empty($apellido2) && !empty($email) && !empty($telefono) && !empty($classSelect)) {
        $sql = "UPDATE alumnos SET nombre = ?, apellido1 = ?, apellido2 = ?, email = ?, telefono = ?, clase = ? WHERE id = ? AND profesor_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssii", $nombre, $apellido1, $apellido2, $email, $telefono, $classSelect, $id, $profesorId);
